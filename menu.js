    ...
    .then(html => {
      ...
      const meals = $(`.heute_${todayStringFormat}`)
        ...
        .map((index, element) => {
          ...
          let name = $ele
            .find(".js-schedule-dish-description")
            .contents()
            .filter(function() {
...
const finanzmensa_menu = () => {
  return new Promise(resolve => {
    resolve(
      new RemoteMenu(
        MENSA_TYPES.FINANZKANTINE,
        "http://fm-kantine.hf-catering.de/"
      )
    );
  });
};
...
const brusko_menu = () => {
  return new Promise(resolve => {
    resolve(
      new RemoteMenu(
        MENSA_TYPES.BRUSKO,
        "http://www.brusko.de/brusko-menu.html"
      )
    );
  });
};
...
const osteria_menu = () => {
  return new Promise(resolve => {
    resolve(
      new RemoteMenu(MENSA_TYPES.BRUSKO, "http://losteria.de/menu/pizza/")
    );
  });
};
